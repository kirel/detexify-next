...
final class WebAppSchemeHandler: NSObject, WKURLSchemeHandler {
    ...
    private static func findWebAppRoot() -> URL? {
        // Prefer app-bundle layouts first. `Bundle.module` is generated by
        // SwiftPM and can assert if its resource bundle is not present in a
        // hand-packaged .app, so keep it as a lazy development fallback only.
        ...
        if let packagedRoot = firstExistingWebAppRoot(in: packagedCandidates) {
            return packagedRoot
        }

        // SwiftPM development layout. Release app bundles should not touch
